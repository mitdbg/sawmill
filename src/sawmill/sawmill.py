...
from .ate import ATECalculator, Pruner
...
class Sawmill:
    ...
    def reject_undecided_outgoing(
        self, src: str, interactive: bool = True
    ) -> Tuple[float, Optional[str], Optional[str]]:
        """
    ...
    def explore_candidate_causes(
        ...
        lasso_alpha: float = Pruner.LASSO_DEFAULT_ALPHA,
        lasso_max_iter: int = Pruner.LASSO_DEFAULT_MAX_ITER,
        skip_lasso: bool = False,
        use_multivariate_regression: bool = False,
        ...
        Parameters:
            ...
            lasso_alpha: The alpha parameter to be used for Lasso regression.
            lasso_max_iter: The maximum number of iterations to be used for Lasso regression.
            skip_lasso: Whether to skip the Lasso step and return all variables as candidates.
            use_multivariate_regression: Whether to use multivariate regression to rank candidates.

        Returns:
            A tuple containing:
            (1) A dataframe containing the candidate causal graph neighbors for `target` and
        ...
        candidates = (
            ...
            if not skip_lasso
            else [c for c in self._prepared_log.columns if c != target]
        )
        ...
        if len(candidates) == 0:
            ...
            return pd.DataFrame(columns=column_order), elapsed
        ...
        if not use_multivariate_regression:
            num_processors = multiprocessing.cpu_count()
            with multiprocessing.Pool(processes=num_processors) as pool:
            ...
            result_df = Regression.multi_ols(
                X_columns, self._prepared_log[X_columns], self.prepared_log[target]
            )
            result_df = (
                result_df.sort_values(by="Normalized Slope", ascending=False)
                .drop(columns=["Normalized Slope"])
                .reset_index(drop=True)
            )
        ...
        ret_val = result_df[column_order]
